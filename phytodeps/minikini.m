function [pointList pointListL pointListE SL SE] = minikini(FileList,para)
    try 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % 
...
%         h = figure;
        ...
        for t = frms
            ...
            % track each point
            for pt = 1:size(pointList,1)        
                % conditional statement for tracking
                if pointList(pt,2,t) > TH
                    T = TR(I,G,pointList(pt,:,t),RADIUS,domainPara{1}.d(1:2,:)',THRESH,initP,1); % track is not tensor ready
                else
                    T = [0 0 0 0 0 0];
                end            
                pointList(pt,:,t+1) = pointList(pt,:,t) + fliplr((T(end-1:end)));
                %pt
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % display to screen
...
%                 figure(h);
                ...
                myimagesc(I);
                hold on;
                plt(fliplr(pointList(:, :, t)), 'r.', 3);
                allpts = 1 : size(pointList,1);
                ycrcs  = arrayfun(@(k) pointList(k, 1, t) + RAD * sin(linspace(-pi, pi, 200)), allpts, 'UniformOutput', 0);
                xcrcs  = arrayfun(@(k) pointList(k, 2, t) + RAD * cos(linspace(-pi, pi, 200)), allpts, 'UniformOutput', 0);
                crcs   = cellfun(@(x,y) [x ; y]', xcrcs, ycrcs, 'UniformOutput', 0);
                cellfun(@(x) plt(x, 'b-', 1), crcs, 'UniformOutput', 0);
                hold off;
                ttl = sprintf('Frame %d of %d', t, numel(frms));
...
%                 plot(pointList(k, 2, t) + RAD * cos(linspace(-pi, pi, 200)), ...
%                     pointList(k, 1, t) + RAD * sin(linspace(-pi, pi, 200)),'b');
